...
class GraphCNN(nn.Module):
    def __init__(self, num_layers, num_mlp_layers, input_dim, hidden_dim, output_dim, final_dropout, learn_eps,
        ...
        self.register_buffer('pre_features', torch.zeros(hidden_dim, hidden_dim))
        self.register_buffer('pre_weight1', torch.ones(hidden_dim, 1))
    ...
    def forward(self, batch_graph):
        ...
        for layer, h in enumerate(hidden_rep):
            ...
            if layer !=0:
                flatten_features+= F.dropout(pooled_h, self.final_dropout,
                                          training=self.training)
            score_over_layer += F.dropout(self.linears_prediction[layer](pooled_h), self.final_dropout,
                                          training=self.training)

        return score_over_layer,flatten_features
